...
type IuputProps = BaseIuputProps &  Omit<InputHTMLAttributes<HTMLElement>, 'size'>
...
const getFilterSuggectionList = (value: string, onSelect: (obj: any) => void) => {
  const list = loadAll();
...
const renderFilterSuggecionList = (list: Array<any>, hoverIndex: number) => {
  return list.map((item, index) => {
    const classes = classNames('item', {
      'is-hover': index === hoverIndex
    })
    return <li className={classes} >{item.value}</li>
  })
}

export const Input: FC<IuputProps> = (props) => {
  const { disabled, size, className, value, suffixIcon, prefixIcon,  prepend, append, children,  ...restProps } = props;
  ...
  const [showSuggection, setShowSuggection] =  useState(true);
  ...
  const filterList = getFilterSuggectionList(val as string, handleClickSuggectionItem);

  const handleKeyUp = (event: React.KeyboardEvent<HTMLInputElement> ) => {
    // console.log(event)
    if(event.code === 'ArrowDown' && hoverIndex < filterList.length - 1){ // keyCode=40 
    ...
    if(event.code === 'Enter' && hoverIndex > 0){ //keyCode=13
      handleClickSuggectionItem(filterList[hoverIndex])
    }
  }
  
  const lisTemplete = renderFilterSuggecionList(filterList, hoverIndex);
  const showLis = showSuggection && filterList.length;
  return (
    <div className="wasabi-input-wrap" >
      <div className={classes}>
        ...
        <input onInput={changeInput} className="input" value={val} onKeyUp={handleKeyUp} disabled = {disabled} {...restProps} />
